 ...
 * cityPicker.init();
 */
;var IndustryPicker = function (options) {
    this.template = $('<div class="hzw-city-picker" id="hzw_industry_picker"><div class="hzw-hot-wrap"><p>热门行业</p><ul id="hzw_hot_city"></ul></div><div class="line"></div><div class="hzw-wrap"><p>选择行业</p><ul class="hzw-province-wrap" id="hzw_province_wrap"></ul></div></div>');
...
IndustryPicker.prototype = {
    ...
    buildProvinceTpl: function () {
        ...
        for (var i = 0, len = province.length; i < len; i++) {
            province_html += '<li class="hzw-province" data-id="' + province[i]['id'] + '" data-name="' + province[i]['name'] + '"><div class="hzw-province-name">' + province[i]['name'] + '</div></li>';
        }
    ...
    provinceEvent: function () {
        ...
        that.province_wrap.on('click', '.hzw-province', function (event) {
            ...
            var pname = _this.data('name');
            that.settings.target.val(pname);
            that.template.remove();

            if (that.settings.callback) that.settings.callback(pid);
